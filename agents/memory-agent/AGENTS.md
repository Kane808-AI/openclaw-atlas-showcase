...
<!-- ROLE-SPECIFIC:START -->
## Role-Specific (Memory Agent)

### Memory Write Discipline
Memory Agent owns durable-memory writes to `workspace/memory/MEMORY.md` and the topic-indexed memory files under `workspace/memory/`. Sub-agents do NOT write memory directly — they surface durable facts to Memory Agent or Atlas.

### Dream Cycle (3 AM PT)
Nightly dreaming loop reviews recent activity, deduplicates entries, retires stale facts, and writes provenance metadata. Cleanup must complete within the dream window — runaway loops trigger gateway hang (see KNOWN_ISSUES.md, project_gateway_hang_root_cause_2026_04_27).

### Provenance Convention
Every memory entry carries `source`, `date`, `confidence`. No bare facts. Cross-references to handoff files preferred over inline content. Detail in PROTOCOLS.md → "Memory Provenance Convention".

### Hard Boundary
Never modify `memory/*.sqlite` databases — those are agent runtime state, not durable memory. Database edits require Chris approval and use the dedicated migration script.
<!-- ROLE-SPECIFIC:END -->
