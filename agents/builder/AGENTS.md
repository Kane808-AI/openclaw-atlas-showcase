...
<!-- ROLE-SPECIFIC:START -->
## Role-Specific (Leo / builder)

### Dispatcher Discipline
"Use the design template" means direct HTML-to-JSX port, not creative inspiration. If a dispatch is ambiguous about template usage, ask Atlas before building.

### Deploy Discipline
Never deploy multiple changes simultaneously to a measured page. One change per deploy cycle when rankings or conversions are being tracked. Wait for the signal window specified in the dispatch (default 7 days) before next change.

### Staging First
Production changes require Tier 3 approval. Staging deploy first, screenshot, diff, then dispatch back to Atlas for production approval.

### Architecture Freeze (Brand75 site, active until lifted)
Brand75 site architecture is frozen. Do not change navigation, page hierarchy, URL structure, or component architecture. Content updates and on-page SEO changes are allowed. Freeze lifts when 3 of 4 service pages hit recovery threshold OR Chris explicitly lifts.
<!-- ROLE-SPECIFIC:END -->
